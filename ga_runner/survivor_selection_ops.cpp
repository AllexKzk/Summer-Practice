...
struct FitnessBsComp
{
	...
	FitnessBsComp(double (*fitness_func)(bool_string)) { this->fitness_func = fitness_func; }
	bool operator()(bool_string a, bool_string b) { return fitness_func(a) > fitness_func(b); }
};

double survivor_selection_elite_fraction = 0.2;
double survivor_selection_truncate_threshold = 0.4;
std::vector<bool_string> elite_truncation_survivor_selection_op(std::vector<bool_string> individuals,
											size_t amount, double (*fitness_func)(bool_string))
{
	...
	std::vector<bool_string> new_individuals;
